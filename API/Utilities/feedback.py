...
# TODO! MAKE SURE THAT Core.schemas NOT USED !!! (flake8)
...
async def get_chatbot_response(query: str, task: str) -> str:
    res = await llm.get_response({"prompt": query,
                                  'temperature': 0.1,
                                  'max_tokens': 300})
    return res
...
async def calculate_metrics(query: str, task: str) -> dict:
    metrics = {
        "criterion_1": await get_concise_focus(query, task),
        "criterion_2": await get_clarity_spec(query, task),
        "criterion_3": await get_relevance_ctx(query, task),
        "criterion_4": await get_purpose_out(query, task)
    }
    return metrics


# calculate_criterion_conciseness_and_focus
async def get_concise_focus(query: str, task: str) -> float:
    prompt = f"""You evaluate user queries for task-solving LLM.
    There is the task: {task}.
    There is the query that user formulated for this problem: {query}
...
async def get_clarity_spec(query: str, task: str) -> float:
    prompt = f"""You evaluate user queries for task-solving LLM.
    There is the task: {task}.
    There is the query that user formulated for this problem: {query}
...
async def get_relevance_ctx(query: str, task: str) -> float:
    prompt = f"""You evaluate user queries for task-solving LLM.
    There is the task: {task}.
    There is the query that user formulated for this problem: {query}
...
async def get_purpose_out(query: str, task: str) -> float:
    prompt = f"""You evaluate user queries for task-solving LLM.
    There is the task: {task}.
    There is the query that user formulated for this problem: {query}
