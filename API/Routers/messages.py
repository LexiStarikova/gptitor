...
def get_all_messages_by_conversation_id(conversation_id: int, 
    ...
    return crud.get_all_messages_by_conversation_id(db=db, conversation_id=conversation_id)
...
async def send_query(conversation_id: int, 
                     ...
                     db: Session = Depends(database.get_db)):
    return await crud.send_query(db=db, conversation_id=conversation_id, query=query, user_id=user_id)
