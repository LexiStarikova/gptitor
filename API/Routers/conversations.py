...
from Core import crud, schemas, models, database
...
def create_new_conversation(llm_id: schemas.LLM_ID, 
    ...
    return crud.create_new_conversation(db=db, llm_id=llm_id.llm_id, user_id=user_id)

@router.delete("/{conversation_id}", response_model=str, status_code=200)
def delete_conversation(conversation_id: int, 
                        user_id: int = Depends(auth.get_current_user), 
                        db: Session = Depends(database.get_db)):
    return crud.delete_conversation(db=db, conversation_id=conversation_id, user_id=user_id)
...
def get_all_conversations(user_id: int = Depends(auth.get_current_user), 
                          db: Session = Depends(database.get_db)):
    return crud.get_all_conversations(db=db, user_id=user_id)